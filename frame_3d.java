...
public class frame_3d extends javax.swing.JFrame {
    ...
    private Pyramid [] p;
    private int nr_of_pyramids;
    private int pyramid_size;
    private boolean init_rand_pyramids = false;
    
    private Plane [] pl;
    private boolean plane_rotation = false;
    private double plane_w = 300;
    private double pl_x = plane_w;
    private double pl_y = 0;
    private double pl_z = 0;
    ...
    private boolean show_cube_pos = true;
    
    private Cube c1;
    
    private Terrain terr;
    private int terrain_width = 90;
    private boolean is_terrain = false;
    private boolean is_move_terrain = false;
    private int terrain_counter = 0;
    private int terrain_modulo = 15;
    ...
    {
        terr = new Terrain(12,terrain_width);
    }
    ...
    public void add_pyramid(double x, double y, double z, double w, boolean rot)
    {
        if(nr_of_pyramids < pyramid_size)
        {
            p[nr_of_pyramids++] = new Pyramid(x,y,z,w,rot);
        }
    }
    ...
    public void rotate_cubes(Vec3D pos)
    {
        for(int ix = 0; ix < nr_of_cubes; ix++)
        {
            ...
            double val = Math.sqrt(2) * c[ix].get_width();
    ...
    public void init_planes()
    {
        ...
        //x
        v4[0] = new Vertex(pl_x - plane_w  , pl_y            ,  pl_z);
        v4[1] = new Vertex(pl_x - plane_w  , pl_y - plane_w  ,  pl_z);
        v4[2] = new Vertex(pl_x - plane_w  , pl_y - plane_w  ,  pl_z - plane_w);
        v4[3] = new Vertex(pl_x - plane_w  , pl_y            ,  pl_z - plane_w);
        
        x = pl_x - plane_w;
        y = (pl_y + (pl_y - plane_w))/2;
        z = (pl_z + (pl_z - plane_w))/2;
        
        pl[0] = new Plane(v4,plane_rotation,x,y,z);
        
        
        //y
        v4[0] = new Vertex(pl_x - plane_w   ,   pl_y  ,   pl_z - plane_w);
        v4[1] = new Vertex(pl_x - plane_w   ,   pl_y  ,   pl_z          );
        v4[2] = new Vertex(pl_x             ,   pl_y  ,   pl_z          );
        v4[3] = new Vertex(pl_x             ,   pl_y  ,   pl_z - plane_w);
        
        x = (pl_x + (pl_x - plane_w))/2;
        y = pl_y;
        z = (pl_z + (pl_z - plane_w))/2;
        ...
        //z
        v4[0] = new Vertex(pl_x             , pl_y               ,  pl_z - plane_w);
        v4[1] = new Vertex(pl_x - plane_w   , pl_y               ,  pl_z - plane_w);
        v4[2] = new Vertex(pl_x - plane_w   , pl_y - plane_w     ,  pl_z - plane_w);
        v4[3] = new Vertex(pl_x             , pl_y - plane_w     ,  pl_z - plane_w);
    ...
    public void init_rand_pyramids()
    {
            nr_of_pyramids = 0;
            pyramid_size = 30;
            p = new Pyramid[pyramid_size];
            for(int ix = 0; ix < 8; ix++)
            {
                double x = -800+rand.nextInt(501);
                double y = -250+rand.nextInt(501);
                double z = -500+rand.nextInt(601);
                double w = 80+rand.nextInt(150);
    ...
    public void init_pyramids()
    {
        nr_of_pyramids = 0;
        pyramid_size = 30;
        p = new Pyramid[pyramid_size];
        if(init_rand_pyramids)
        {
            init_rand_pyramids();
        }
    }
        ...
        ActionListener taskPerformer = new ActionListener()
                {
                    ...
                    public void actionPerformed(ActionEvent evt)
                    {
                        ...
                        for(int ix = 0; ix < nr_of_pyramids; ix++)
                        {
                            pyramids_rotation(ix);
                            draw_pyramid(ix);
                        }
                        ...
                        if(is_terrain)
                        {
                            ...
                            {
                                terrain_counter++;
                                if(terrain_counter % terrain_modulo == 0)
                                {
                                    terr.move_terrain();
                                }
                            }
    ...
    public void draw_terrain()
    {
        ...
        for(int ix = 0; ix < terr.tot_triangles(); ix++)
        {
                ...
                for(int x = 0; x < 4; x++)
                {
                    ...
                    double [] cords1 = draw_cords(terr.get_Vertex(ix, k));
                    
                    
                    cords1[0] *= 1.0 / (z_far - z_near);
                    cords1[1] *= 1.0 / (z_far - z_near);
                ...
                int col = (int)(eval_terrain_light(ix)*355);  
                if(col > 255)
                {
                    col = 255;
                }
                ...
                if(within && is_tri_front(terr.get_triangle(ix)))
                {
    ...
    public boolean is_tri_front(Triangle t)
    {
        boolean is_front = false;
        Vec3D front = pos.add(forward.scale(10));
        Vec3D back = pos.subtract(forward.scale(10));
        Vec3D plane_vec = new Vec3D(t.get_vertex(0).get_x(),t.get_vertex(0).get_y(),t.get_vertex(0).get_z());
    ...
    public void draw_cords_string()
    {
        Font tr = new Font("TimesRoman", Font.PLAIN, 18);
        g.setFont(tr);
        g.setColor(Color.white);
        g.drawString("x "+(int)pos.x() + "   y "+(int)pos.y()+"   z "+(int)pos.z(), FRAME_WIDTH-200, 50);
    }
    ...
    public double eval_terrain_light(int tri)
    {
        for(int ix = 0; ix < 3; ix++)
        {
            light = light.normalize();
        ...
        double dp = terr.get_triangle(tri).x_norm_normal() * light.x() + terr.get_triangle(tri).y_norm_normal() * light.y() + terr.get_triangle(tri).z_norm_normal() * light.z();
        return dp;
    }
    
    public double eval_pyr_light(int pyr_ix, int tri)
    {
        for(int ix = 0; ix < 3; ix++)
        {
            light = light.normalize();
        ...
        double dp = p[pyr_ix].get_triangle(tri).x_norm_normal() * light.x() + p[pyr_ix].get_triangle(tri).y_norm_normal() * light.y() + p[pyr_ix].get_triangle(tri).z_norm_normal() * light.z();
        return dp;
    }
    ...
    public void sort_pyramids_dist()
    {
        boolean swap;
        do
        {
            swap = false;
            for(int ix = 0; ix < nr_of_pyramids - 1; ix++)
            {
                Vec3D d = new Vec3D();
                Vec3D c0 = new Vec3D(p[ix].get_cube_cent_x(),p[ix].get_cube_cent_y(),p[ix].get_cube_cent_z());
                Vec3D c1 = new Vec3D(p[ix+1].get_cube_cent_x(),p[ix+1].get_cube_cent_y(),p[ix+1].get_cube_cent_z());
    ...
    public boolean is_pyramid_front(Pyramid p)
    {
        boolean is_front = false;
        Vec3D front = pos.add(forward.scale(10));
        Vec3D back = pos.subtract(forward.scale(10));
        Vec3D cube_vec = new Vec3D(p.get_cube_cent_x(),p.get_cube_cent_y(),p.get_cube_cent_z());
        ...
        else if (!is_global_rotation) 
        {
            ...
            for (int ix = 0; ix < nr_of_cubes; ix++) 
            {
                ...
                if(add_cube_orthogonal)
                {
                    ...
                    c[ix] = new Cube(x0,y0,z0,w,add_cube_rotation);
                }
            }
        }
    ...
    public void draw_coming_cube()
    {
        Vec3D cube_cords = new Vec3D();
        cube_cords = pos.add(forward.scale(300));
        double x1 = cube_cords.x();
        double y1 = cube_cords.y();
        double z1 = cube_cords.z();
        double w1 = add_cube_width;
        
        
        
        if(add_grid && !add_cube_rotation)
        {
            int x_val = (int)(x1 - ((int)(x1) % (int)(w1)));
            int y_val = (int)(y1 - ((int)(y1) % (int)(w1)));
            int z_val = (int)(z1 - ((int)(z1) % (int)(w1)));
            c1 = new Cube(x_val,y_val,z_val,w1,add_cube_rotation);
        }
        else
        {
            c1 = new Cube(x1,y1,z1,w1,add_cube_rotation);
        }
        ...
        for (int ix = 0; ix < 12; ix++) 
        {
            ...
            for (int x = 0; x < 4; x++) 
            {
                ...
                double[] cords1 = draw_cords(c1.get_Vertex(ix, k));
            ...
            for (int k = 0; k < 3; k++) 
            {
                ...
                k2 = k2 % 3;
                double[] cords1 = draw_cords(c1.get_Vertex(ix, k));
                double[] cords2 = draw_cords(c1.get_Vertex(ix, k2));

                cords1[0] *= 1.0 / (z_far - z_near);
                cords1[1] *= 1.0 / (z_far - z_near);
                cords2[0] *= 1.0 / (z_far - z_near);
                cords2[1] *= 1.0 / (z_far - z_near);
    ...
    public void bubble_sort_pyr(Pyramid p)
    {
        Triangle [] tri = p.get_triangles();
        ...
        do
        {
            ...
            for(int ix = 0; ix < 5; ix++)
            {
                ...
                for(int t = 0; t < 3; t++)
                {
                    double dx = Math.abs(tri[ix].get_vertex_x(t) - pos.x());
                    double dy = Math.abs(tri[ix].get_vertex_y(t) - pos.y());
                    double dz = Math.abs(tri[ix].get_vertex_z(t) - pos.z());
                ...
                for(int t = 0; t < 3; t++)
                {
                    double dx = Math.abs(tri[ix+1].get_vertex_x(t) - pos.x());
                    double dy = Math.abs(tri[ix+1].get_vertex_y(t) - pos.y());
                    double dz = Math.abs(tri[ix+1].get_vertex_z(t) - pos.z());
                    double dist = Math.sqrt((dx*dx)+(dy*dy)+(dz*dz));
                    sum1 += dist;
                }
                sum1 /= 3;
    ...
    public void draw_pyramid(int pyr_ix)
    {
        bubble_sort_pyr(p[pyr_ix]);
        ...
        for(int ix = 0; ix < 6; ix++)
        {
                ...
                for(int x = 0; x < 4; x++)
                {
                    ...
                    double [] cords1 = draw_cords(p[pyr_ix].get_Vertex(ix, k));
                ...
                int col = (int)(eval_pyr_light(pyr_ix,ix)*355);  
                col *= -1;
                if(col < 0)
                {
                    col = 0;
                ...
                if(is_pyramid_front(p[pyr_ix]) && within)
                {
                   g.fillPolygon(x_p,y_p,4);
                }
    ...
    public void draw(int cubes_ix)
    {
        ...
        for(int ix = 0; ix < 12; ix++)
        {
                ...
                for(int k  = 0; k < 3; k++)
                {
                    ...
                    g.setColor(Color.white);
                    if(is_z_visible(cubes_ix,ix) && is_cube_front(c[cubes_ix])&& draw_lines && within)
                    {
                        g.drawLine((int)cords1[0],(int)cords1[1],(int)cords2[0],(int)cords2[1]);
                    }
    ...
    public void pyramids_rotation(int ix)
    {
        p[ix].x_rotate();
        p[ix].y_rotate();
        p[ix].z_rotate();
    ...
    private void initComponents() {
        ...
        jCheckBox11.setFont(new java.awt.Font("Tahoma", 0, 22)); // NOI18N
        jCheckBox11.setSelected(true);
        jCheckBox11.setText("show cube pos");
        ...
        jCheckBox13.setText("move terrain");
        ...
        jCheckBox10.setText("init rand pyramids");
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(layout.createSequentialGroup()
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addComponent(jCheckBox11, javax.swing.GroupLayout.PREFERRED_SIZE, 320, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addGroup(layout.createSequentialGroup()
                                .addGap(2, 2, 2)
    ...
    private void formMouseWheelMoved(java.awt.event.MouseWheelEvent evt) {//GEN-FIRST:event_formMouseWheelMoved
        ...
        if(evt.getWheelRotation() == 1 && pr_angle < 1.8)
        {
            pr_angle += Math.PI/128;
        }
        else if(evt.getWheelRotation() == -1 && pr_angle > 1.2)
        {
            pr_angle -= Math.PI/128;
        }
    ...
    private void jCheckBox13ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCheckBox13ActionPerformed
        ...
        {
            is_move_terrain = true;
        }
        else
        {
            is_move_terrain = false;
        }
    ...
    private void jSlider4StateChanged(javax.swing.event.ChangeEvent evt) {//GEN-FIRST:event_jSlider4StateChanged
        ...
        terrain_width = jSlider4.getValue();
        jLabel4.setText("terrain width "+terrain_width);
        init_terrain();
    ...
    private void jCheckBox10ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCheckBox10ActionPerformed
        ...
        {
            init_rand_pyramids = true;
        }
        else
        {
            init_rand_pyramids = false;
        }
        init_pyramids();
